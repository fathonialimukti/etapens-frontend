<template>
  <template v-if=" project ">
    ...
    <a v-if=" project.url " :href=" project.url " target="_blank">
      <v-btn variant="elevated" color="secondary" rounded="lg">Website</v-btn>
    </a>
    <a v-if=" project.sourceCode " :href=" project.sourceCode " target="_blank">
      <v-btn variant="elevated" color="secondary" rounded="lg">Source Code</v-btn>
    </a>
      ...
      <dd class=" text-gray-900 mt-0 col-span-2">
        <ul role="list"
          ...
          <li
            ...
            :key=" document " class="pl-3 pr-4 py-3 flex items-center justify-between ">
...
export default {
  ...
  async created () {
    await axios.get( projectService + 'get-project', { params: { title: this.$route.params.title } } ).then( ( r ) => this.project = r.data )
  },
  methods: {
    postComment () {
      axios
