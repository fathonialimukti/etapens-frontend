<template>
  <div class="grid md:grid-cols-9 gap-2">
    ...
    <v-text-field v-model=" name " label="Title" class="md:col-span-5" @change=" getData "
  ...
  <v-table fixed-header>
    <thead>
      <tr>
        ...
        <th class="text-left">
          <h1 class=" font-bold text-lg">Port</h1>
        </th>
        <th class="text-left">
    ...
    <tbody>
      <tr v-for="  backend  in data" :key=" backend.id ">
        ...
        <td>
          <v-btn v-if=" !backend.isActive " @click=" deploy( backend ) " color="error" size="small" rounded="pill">
            Deploy
          </v-btn>
