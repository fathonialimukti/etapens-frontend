    ...
    <v-menu>
      ...
      <v-list>
        <router-link v-if=" !user.project " :to=" { name: 'Create Project' } ">
          <v-list-item>
            <v-list-item-title>Daftarkan Tugas Akhir</v-list-item-title>
          </v-list-item>
        </router-link>

        <router-link v-if=" !user.backend " :to=" { name: 'Create Backend' } ">
          <v-list-item>
            <v-list-item-title>Buat Backend</v-list-item-title>
          </v-list-item>
        </router-link>

        <router-link v-if=" !user.database " :to=" { name: 'Create Database' } ">
          <v-list-item>
            <v-list-item-title>Request Database</v-list-item-title>
          </v-list-item>
        </router-link>
      </v-list>
    </v-menu>
  ...
  <v-expansion-panels variant="popout" class="my-4">
    ...
    <v-expansion-panel v-if=" user.project " title="Project" color="secondary">
      <v-expansion-panel-text>
        ...
        <v-card-text>
          <v-list lines="one">
            <v-list-item @click="" v-for="lecturer in user.project.lecturers" :key=" lecturer.id "
              :title=" lecturer.name " :subtitle=" lecturer.nip " :prepend-avatar=" lecturer.image "></v-list-item>
          </v-list>
        </v-card-text>

        <div v-if=" user.project.sourceCode ">
          <v-card-title> Repository </v-card-title>
          <v-card-text>{{ user.project.sourceCode }}</v-card-text>
        </div>

        <div v-if=" user.project.url ">
    ...
    <v-expansion-panel v-if=" user.database " title="Database" color="secondary">
      <v-expansion-panel-text>
        ...
        <v-card-text
          v-if=" user.database.type == 'mysql' ">ec2-52-74-228-86.ap-southeast-1.compute.amazonaws.com:3306</v-card-text>
        <v-card-text
          v-if=" user.database.type == 'postgres' ">ec2-52-74-228-86.ap-southeast-1.compute.amazonaws.com:5432</v-card-text>
